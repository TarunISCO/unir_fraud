...
data.drop(['Tx count for remitter', 'Tx count for beneficiary', 'Days count from the last tx',
           'IS_First_Tx for Beneficiary'], axis=1, inplace=True)

# scalar = StandardScaler()
# data[['Tx count for remitter', 'Tx count for beneficiary', 'Days count from the last tx',
#       'IS_First_Tx for Beneficiary']] = scalar.fit_transform(data[['Tx count for remitter', 'Tx count for beneficiary', 'Days count from the last tx',
#                                'IS_First_Tx for Beneficiary']])
...
label_df = pd.DataFrame(data=dict(y_test=y_test, y_pred=y_pred_, y_prob=y_prob[:, 1], amount=data_test['Amount'], sending_method=data_test['Sending Method Type']))
print(label_df.dtypes)
print(label_df['sending_method'])
...
def amount_description(start_prob, end_prob):
    ...
    fn_bank_account = false_negative[(false_negative['y_prob'] > start_prob) & (false_negative['sending_method'].isin(['BANK ACCOUNT']))]

    fp_bank_account = false_positive[(false_positive['y_prob'] < end_prob) & (false_positive['sending_method'].isin(['BANK ACCOUNT']))]
    ...
    print('\n\nFalse Negative with Sending Method = Bank Account', fn_bank_account.shape[0], fn_bank_account['amount'].sum())
