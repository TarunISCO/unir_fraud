...
"""
Train on single file with different feature sets and different samplings
"""

data_ = pd.read_excel('data/Transformed June/Transformed-CCDC-1-2017_6-2019.xlsx')
...
for new_features, user_level in itertools.product([True, False], [True, False]):
    ...
    data_test = data[(data['Tx date and time'] >= '2019-05-01') & (data['Tx date and time'] < '2019-06-01')]
    data_train = data[(data['Tx date and time'] >= '2017-01-01') & (data['Tx date and time'] < '2019-05-01')]
    ...
    for sampling in [5, 6, 7]:
        ...
        plt_title = "CCD_basic{is_new}{is_user}_{samp}".format(is_new='+new' if new_features else '',
                                                               is_user='+user_level' if user_level else '',
                                                               samp='sampling=' + str(sampling))

        ax.set_title(plt_title)
        cnf_path = f'observations/july/CCDC_july/{plt_title}.png'
