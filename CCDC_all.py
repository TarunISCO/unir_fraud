...
for new_features, user_level, d in itertools.product([True, False], [True, False], ['jan2017', 'apr2017', 'jan2018']):
    data = pd.read_excel(f'data/july/{d}/Transformed-CCDC.xlsx')
    ...
    features = ['Age_transformed', 'Amount_transformed', 'IP_RISK_LEVEL_ID=4', 'FINAL_RESULT_EMAILAGE=FAIL',
                ...
                'FINAL_RESULT_EMAILAGE=PASS',
                'DOMAIN_NAME_IV', 'IP_RISK_LEVEL_ID=5', 'IP_USER_TYPE_IV', 'Joining Time_transformed', 'Label',
                'Tx date and time', 'Amount']
    ...
    for sampling in [5, 6, 7]:
        ...
        plt_title = "CCDC_basic{is_new}{is_user}_{samp}".format(is_new='+new' if new_features else '',
        ...
        cnf_path = f'observations/july/CCDC/{plt_title}.png'
